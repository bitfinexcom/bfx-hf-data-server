...
const execDazaar = require('./cmds/exec_dazaar')
const manageCores = require('./cmds/manage_cores')
...
const COMMANDS = {
  'manage.cores': manageCores,
  'exec.dazaar': execDazaar,
  'exec.str': execStr,
  'exec.bt': execBT,
  'get.bts': getBTs,
...
class DataServer {
  ...
  constructor ({
    ...
    assert.ok(dir, 'dir must be set')
    assert.ok(db, 'db must be set')

    this.createDazaarDirectory(dir)
  ...
  createDazaarDirectory (dir) {
    const dPath = path.join(dir, 'dazaar')
