...
const COMMANDS = {
  ...
  'get.bt.history.details': getBtData,
...
class DataServer {
  ...
  constructor ({
    ...
    if (sqlitePath) sqliteDb.connectDb(sqlitePath)
  }
  ...
  expectSync ({ exchange, symbol, tf, start, end }) {
    ...
    if (!this.activeSyncs[key]) {
      const msg = `error: tried to expect non-existent sync (${key})`
      debug(msg)
