...
module.exports = async (ds, ws, msg) => {
  ...
  if (includeCandles) {
    ...
    if (candleData.length > 0) {
      priceFeed.update(new BigNumber(candleData[0][candlePrice]))
    }
  ...
  const progressCallback = (currentTick, totalTicks) => {
    if ((currentTick % 10) === 0 || currentTick === totalTicks) {
      send(ws, ['bt.progress', currentTick, totalTicks, meta])
    }
  }
  ...
  execOffline(strategy, args, progressCallback).then((btState = {}) => {
