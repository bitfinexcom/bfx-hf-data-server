...
module.exports = async (ds, ws, msg) => {
  ...
  const [
    ...
    exchange, strategyId, startTs, endTs, symbol, timeframe, includeCandles, includeTrades, candleSeed, sync = true, margin = false, strategyContent, executionId, constraints = {}, leverageSettings = {}, stopOrderSettings = {}
  ] = msg[1] || []
  const { capitalAllocation, stopLossPerc, maxDrawdownPerc } = constraints
  const { useMaxLeverage = false, increaseLeverage = false, leverage = 0 } = leverageSettings
  const { addStopOrder = false, stopOrderPercent = 0 } = stopOrderSettings

  // align start and end times with candle mts range
  const start = alignRangeMts(timeframe, startTs)
  const end = alignRangeMts(timeframe, endTs)
