...
const PriceFeed = require('bfx-hf-strategy-perf/src/PriceFeed')
const PerformanceManager = require('bfx-hf-strategy-perf/src/PerformanceManager')
const startPerformanceWatchers = require('bfx-hf-strategy-perf/src/StartWatchers')
...
module.exports = async (ds, ws, msg) => {
  ...
  startPerformanceWatchers(perfManager, constraints)
  ...
  if (includeCandles) {
    ...
    priceFeed.update(new BigNumber(candleData[0].close))
  ...
  if (includeTrades) {
    ...
    priceFeed.update(new BigNumber(tradeData[0].price))
